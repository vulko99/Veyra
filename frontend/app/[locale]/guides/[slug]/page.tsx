...
export function generateMetadata({
  ...
  if (!g) return {};
  // Guides are Bulgarian prose with no translation, so the canonical is the
  // Bulgarian URL from either locale and the /en twin stays out of the index —
  // it exists only so an English reader following a link keeps English chrome.
  const canonical = `/guides/${g.slug}`;
  const english = params.locale !== defaultLocale;
  return {
    title: g.metaTitle,
    description: g.description,
    alternates: { canonical },
    robots: english ? { index: false, follow: true } : undefined,
    openGraph: {
      type: "article",
      title: g.metaTitle,
      description: g.description,
...
export default function GuidePage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="relative">
      ...
      <article lang="bg" className="container-x max-w-2xl py-16 sm:py-20">
